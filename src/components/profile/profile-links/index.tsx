 ...
 *
 * @prop {User} user - An object containing the user's information, including optional social media links (e.g., GitHub, LinkedIn, Instagram). The `user` object is expected to adhere to the `User` interface defined in `src/types/user.ts` (or the equivalent path).
 * @prop {boolean} isPublic - A boolean value indicating whether the profile is public or not.
 *
 * @returns {JSX.Element}
            ...
            <div className="py-1 px-4 bg-background w-full items-center mx-auto flex flex-row justify-between rounded-lg ">
                ...
                <div className="flex flex-row items-center gap-4">
                    {user?.github ? (
                        ...
                        <div className="transition-all duration-500 hover:-translate-x-1 text-text hover:text-primary font-medium text-sm">
                            <p>{BeutifyLink(user?.github || "empty")}</p>
                        </div>
                    )}
